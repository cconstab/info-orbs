...
ClockWidgetAlt::ClockWidgetAlt(ScreenManager& manager) : Widget(manager) {
}

ClockWidgetAlt::~ClockWidgetAlt() {
}

void ClockWidgetAlt::setup() {
...
void ClockWidgetAlt::draw(bool force) {
    ...
    if (m_secondSingle != m_lastSecondSingle || force) {
        if (m_secondSingle % 2 == 0) {
            displayDidget(2, ":", 7, 5, ALT_FOREGROUND_COLOR, false);
        } else {
        ...
        if (!FORMAT_24_HOUR && SHOW_AM_PM_INDICATOR) {
            displayAmPm(ALT_FOREGROUND_COLOR);
        }
    }
}

void ClockWidgetAlt::displayAmPm(uint32_t color) {
    GlobalTime* time = GlobalTime::getInstance();
...
void ClockWidgetAlt::update(bool force) {
...
void ClockWidgetAlt::changeMode() {
    GlobalTime* time = GlobalTime::getInstance();
    time->setFormat24Hour( !time->getFormat24Hour() );
    draw(true);
}

void ClockWidgetAlt::displayDidget(int displayIndex, const String& didget, int font, int fontSize, uint32_t color, bool shadowing) {
...
void ClockWidgetAlt::displayDidget(int displayIndex, const String& didget, int font, int fontSize, uint32_t color) {
    this->displayDidget(displayIndex, didget, font, fontSize, color, SHADOWING);
}

void ClockWidgetAlt::displaySeconds(int displayIndex, int seconds, int color) {
    m_manager.reset();
    m_manager.selectScreen(displayIndex);
    TFT_eSPI& display = m_manager.getDisplay();
    if (seconds < 30) {
